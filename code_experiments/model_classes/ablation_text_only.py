...
class TextOnly(torch.nn.Module):
	...
	def _create_network(self, num_classes, num_emb_dim, **model_parameters):
		self.attention_block1 = AttentionBlock(dim_emb=num_emb_dim, num_heads=4, dropout=0.2)
		self.attention_block2 = AttentionBlock(dim_emb=num_emb_dim, num_heads=4, dropout=0.2)
		self.attention_blocks = ModuleList([self.attention_block1, self.attention_block2])

		self.classification_head = Linear(num_emb_dim * 1, num_classes)
	...
	def forward(self, batch: DataBatch):
		...
		for item_claim_emb, item_ev_emb in zip(claim_hidden_states, evidence_reports_hidden_states):
		...
		logits = self.classification_head(batch_fusion_embeddings)

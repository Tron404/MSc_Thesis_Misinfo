...
from torch_geometric.nn import GATv2Conv, global_mean_pool, to_hetero
...
class AttentionBlock(torch.nn.Module):
	def __init__(self, dim_emb, num_heads, dropout):
		super().__init__()

		self.mha = MultiheadAttention(
			embed_dim=dim_emb, num_heads=num_heads, dropout=dropout, batch_first=True
		)
		intra_emb = dim_emb * 4
		self.ffnn = Sequential(
			Linear(dim_emb, intra_emb, bias=True),
			ReLU(),
			Dropout(0.2),
			Linear(intra_emb, dim_emb, bias=True),
		)
		self.layer_norm1 = LayerNorm(dim_emb)
		self.layer_norm2 = LayerNorm(dim_emb)

	def forward(self, claim_emb: torch.Tensor, evidence_emb: torch.Tensor):
		mha_output, attn_weights = self.mha(claim_emb, evidence_emb, evidence_emb)
		attn_weights = attn_weights.squeeze(dim=1)
		# B,L,D
		mha_output = self.layer_norm1(torch.add(mha_output, claim_emb))

		x = self.ffnn(mha_output)
		x = self.layer_norm2(torch.add(x, mha_output))

		return x, attn_weights


class FcGalm(torch.nn.Module):
	...
	def _create_network(self, num_classes, num_emb_dim, **model_parameters):
		num_feat1 = 64
		self.attention_block1 = AttentionBlock(
			dim_emb=num_emb_dim + num_feat1, num_heads=4, dropout=0.2
		)
		self.attention_block2 = AttentionBlock(
			dim_emb=num_emb_dim + num_feat1, num_heads=4, dropout=0.2
		)
		self.attention_blocks = ModuleList([self.attention_block1, self.attention_block2])
		...
		#!! this if more parameter efficient than individual convs with HeteroConv
		self.graph_model = to_hetero(self.graph_model, self.data_metadata, aggr="sum")

		self.classification_head = Linear(num_emb_dim + num_feat1 + num_feat1, num_classes)
	...
	def _get_embeddings(self, batch: DataBatch):
		...
		batched_graph_data = self.make_graph_data(
	...
	def forward(self, batch: DataBatch):
		...
		batch_dict = batched_graph_data.batch_dict
		...
		claim_hidden_states = torch.cat([claim_hidden_states, graph_emb_claim], dim=-1)

		### combine all embeddings
		batch_fusion_embeddings = []
		batch_attention_scores = []
		for batch_idx, (item_claim_emb, item_ev_emb) in enumerate(
			zip(claim_hidden_states, evidence_reports_hidden_states)
		):
			idx_ev_node_batch = torch.where(batch_dict["evidence"] == batch_idx)
			item_ev_emb = torch.cat(
				[item_ev_emb, graph_emb_dict["evidence"][idx_ev_node_batch]], dim=-1
			)
			fusion_embedding1, att_scores1 = self.attention_block1(
				item_claim_emb.unsqueeze(0), item_ev_emb
			)
			fusion_embedding2, att_scores2 = self.attention_block2(fusion_embedding1, item_ev_emb)
			fusion_embedding2 = fusion_embedding2.squeeze(dim=0)

			batch_fusion_embeddings += [fusion_embedding2]
			batch_attention_scores += [torch.cat([att_scores1, att_scores2], dim=0).tolist()]
		batch_fusion_embeddings = torch.stack(batch_fusion_embeddings, dim=0)

		final_concat_emb = torch.cat([batch_fusion_embeddings, graph_emb], dim=1)

		logits = self.classification_head(final_concat_emb)

		return logits, {"attention_scores": batch_attention_scores}
